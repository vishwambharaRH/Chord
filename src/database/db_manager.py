...
class DBManager:
    ...
    def upsert_track(self, title: str, artist: str, album: str | None = None,
                      duration_ms: int | None = None, art_url: str | None = None) -> int:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO tracks (title, artist, album, duration_ms, art_url)
                VALUES (?, ?, ?, ?, ?)
                ON CONFLICT (title, artist, album) DO UPDATE SET
                    duration_ms = COALESCE(excluded.duration_ms, tracks.duration_ms),
                    art_url = COALESCE(excluded.art_url, tracks.art_url)
                """,
                (title, artist, album, duration_ms, art_url),
            )
            row = conn.execute(
                "SELECT id FROM tracks WHERE title = ? AND artist = ? AND album IS ?",
                (title, artist, album),
            ).fetchone()
            return row["id"]
    ...
    def fetch_top_tracks(self, limit: int = 20):
        with self.connect() as conn:
            return conn.execute(
                """
    ...
    def fetch_recent_plays(self, limit: int = 50):
        with self.connect() as conn:
            return conn.execute(
                """
